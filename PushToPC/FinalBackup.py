...
def uivalid():
    realport = port.get()
    if realport.isdigit() == True:
        global uivalidport
        uivalidport = int(realport)
        if uivalidport < 65000:
            proc.start()
        else:
            tkMessageBox.showwarning("Hold on a sec...", "Please enter a number less than 65000 for a port.")
        return 
    else:
        tkMessageBox.showwarning("Hold on a sec...", "Please enter a number only for a port.")
        return 
...
def server():
    ...
    server_address = ("", uivalidport) #Current machine's IP and random open port.
